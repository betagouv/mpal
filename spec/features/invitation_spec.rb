...
feature "Invitation" do
  ...
  let!(:intervenant) { FactoryGirl.create(:intervenant, departements: [projet.departement]) }
  ...
  scenario "prise de contact avec un opérateur" do
    ...
    click_button I18n.t('invitations.nouvelle.action', intervenant: intervenant.raison_sociale)
    expect(page).to have_content(I18n.t('invitations.messages.succes', intervenant: intervenant.raison_sociale))
    expect(page).to have_content('martin@gmel.com')
    expect(page).to have_css '.invites'
    expect(page).to have_content(I18n.t('evenements.invitation_intervenant', intervenant: intervenant.raison_sociale))
  end
