  ...
  task migrate_intervenants: :environment do
    ...
    new_intervenants.each do |new_intervenant|
      ...
      if intervenant.blank?
        Intervenant.create! raison_sociale: new_intervenant[:raison_sociale], clavis_service_id: new_intervenant[:clavis_service_id], adresse_postale: new_intervenant[:adresse_postale], email: new_intervenant[:email], roles: new_intervenant[:roles]
      else
