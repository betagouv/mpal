...
#CONCERNANT LE DOSSIER DE PAIEMENT
  def met_a_jour_statut_paiement(payment)
    payment.statut = :en_cours_d_instruction
  end

  def serialize_dossier_paiement(projet, payment)
    {
    "typeDePaiement": payment.type_paiement,
    #TODO : Verifier quelle date c'est (comme pour dossier de depot, ou date en_cours_instruction)
    # "dateDeDemande": serialize_date(payment.date_depot_demande_paiement)
    "utiIdClavis": projet.agent_instructeur.clavis_id,
    "montantDuPaiement": payment.montant,
    }
  end
end
