class DossiersController < ApplicationController
  ...
  def indicateurs
    ...
    @projets_count = projets.count.count
  ...
  def fill_tab_intervenant all
    ...
    if current_agent.pris?
      ...
      @others = []
      @new_msg = all.where.not("projets.statut >= 2 and projets.operateur_id is not NULL and (projets.eligibilite = 3 or projets.eligibilite = 0)").where(new_msg)
      @rfrn2 = []
      ...
      @others = []
      @new_msg = all.where.not("projets.statut >= 5 and (projets.eligibilite = 3 or projets.eligibilite = 0)").where(new_msg)
...
def render_index
    params.permit(:format, :page, :per_page, :page_noel, :page_noelre, :page_noelco, :page_rfrn2, :page_actif, :page_others, :page_new_msg, :page_verif, :page_action, :page_traited, search: [:query, :status, :sort_by, :type, :folder, :tenant, :location, :interv, :operation_programmee, :from, :to, :advanced, :hma, :activeTab])
